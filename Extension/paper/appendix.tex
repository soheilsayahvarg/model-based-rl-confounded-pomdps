...
\section{Horizon Dilution: Numbers}
\label{app:dilution}

The values plotted in \cref{fig:dilution}(a). Entries are the floor
$\beta\beta_g$ divided by the spread of true values across six candidate
policies. The floor itself is constant along each row.
...
Per-stage leakages behind \cref{cor:selfheal}: at $t=1$ the span is $2$ or $3$
against $|S|=4$, with $\beta$ between $0.51$ and $0.71$; at every $t \ge 2$ the
span reaches $|S|$ and $\beta_t \le 2.7\times10^{-15}$, giving a per-stage floor
below $4.3\times10^{-30}$.
