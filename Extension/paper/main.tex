...
The characterization is a statement about the \emph{first} stage: from $t=2$ on,
history restores per-action completeness and the later blocks stop leaking, so an
inadequate control is partially self-healing along the horizon.
Placing the schedule of \citet{hong2024} inside this family shows its governing
exponent is positive for \emph{every} admissible smoothness ($30/30$ cells), so
the region it prescribes is non-contracting on the leaky branch. At the decision
level, over $20$ seeds, pessimistic and plug-in selection have opposite-signed
regret slopes under that schedule and their curves cross: pessimism helps at
$N{=}2{,}000$ and hurts at $N{\ge}32{,}000$. We are explicit that all of this
lives outside the anchor paper's own assumptions, which is what makes it a
robustness result about the boundary of applicability rather than a defect.
\end{abstract}
...
This narrows the claim and sharpens it. The dimensional test $|O_0|<|S|$ is a
statement about the first stage, not about the whole horizon, and a floor in a
multi-stage value is carried by the stage-1 blocks inside it. That is still
enough to move the decision --- \cref{sec:decision} runs at $T=3$ --- but a
practitioner reading ``check whether $|O_0|<|S|$'' should understand it as
diagnosing the first stage and should expect the damage to be bounded by that
stage's contribution rather than to compound along the horizon.
...
\paragraph{The switches are a stage-1 statement.} \Cref{cor:selfheal} bounds how
far the characterization reaches: from $t=2$ on, history restores per-action
completeness and the later blocks do not leak. We have verified this only in the
population and only at $t=2$; whether the restoration is uniform in $T$, and how
quickly the $t=1$ contribution is diluted as the horizon grows, are open.
