...
cross: pessimism helps at $N{=}2{,}000$ and hurts at $N{\ge}32{,}000$. The floor itself is
diluted by the horizon --- it is constant in $T$ while the value spread grows, so
their ratio decays as $1/T$ --- but we predicted and then refuted the natural
inference that long horizons are therefore safe: at $T=6$ normalized regret is
flat against $T=3$ and raw regret doubles, because the selector commits the same
error and only its price scales. All of this lives outside the anchor paper's own assumptions,
