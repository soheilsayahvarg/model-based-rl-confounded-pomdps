    ...
    for kappa in (1.0, 0.0):
        ...
        for N in N_LIST:
            ...
            recovery.setdefault(f"kappa={kappa}", {})[N] = dict(
                ...
                bR_rmse_ok_cells_std=float(np.std(rec_err)),
                ok_cell_frac=float(np.mean(ok_frac)),
                ok_cell_frac_std=float(np.std(ok_frac)),
                ok_mass_frac=float(np.mean(mass_frac)),      # audit B: honest metric
                ok_mass_frac_std=float(np.std(mass_frac)),
                n_seeds=n_seeds)
            tab = {}
            for n in cands:
                pl = np.array(rows[n]["pl"])
                nv = np.array(rows[n]["nv"])
                bias_pl = pl - v_true[n]
                bias_nv = nv - v_true[n]
                # normal-approx 95% CI on the across-seed mean (n_seeds small; a
                # rough presentation-grade interval, not a claim of asymptotic
                # normality of the estimator itself)
                ci95_pl = 1.96 * float(np.std(bias_pl, ddof=1)) / np.sqrt(n_seeds) \
                    if n_seeds > 1 else float("nan")
                tab[n] = dict(
                    V_true=round(v_true[n], 6),
                    V_naive_pop=round(v_pop_naive[n], 6),
                    V_naive_emp=round(float(nv.mean()), 6),
                    V_naive_emp_std=round(float(nv.std()), 6),
                    V_plugin=round(float(pl.mean()), 6),
                    V_plugin_std=round(float(pl.std()), 6),
                    bias_naive_pop=round(v_pop_naive[n] - v_true[n], 6),
                    bias_naive_emp=round(float(bias_nv.mean()), 6),
                    bias_plugin=round(float(bias_pl.mean()), 6),
                    bias_plugin_std=round(float(bias_pl.std()), 6),
                    bias_plugin_ci95_halfwidth=round(ci95_pl, 6),
                    n_seeds=n_seeds)
