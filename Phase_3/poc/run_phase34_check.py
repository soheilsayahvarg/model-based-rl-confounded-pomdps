...
Checks:
  ...
  [V2] Plug-in with ESTIMATED bridges + empirical p_hat(o_1): the plug-in removes
       ...
       and shrinks with N (N = 5k, 20k, 50k; 5 seeds).
...
def main():
    ...
    for N in (5_000, 20_000, 50_000):
        ...
        debias[N] = {n: dict(
            ...
            bias_plugin_std=float(np.std(r["bias_plugin"])),
            bias_plugin_ci95_halfwidth=float(
                1.96 * np.std(r["bias_plugin"], ddof=1) / np.sqrt(n_seeds)),
    ...
    print(f"\n[V2] plug-in de-biasing (mean over {len(SEEDS)} seeds)")
    ...
    for variant, proj in (("vanilla", False), ("projected", True)):
        ...
        for c, recs in sweep.items():
            subopt_pess_v = [r["suboptimality_pessimistic"] for r in recs]
            agg[c] = dict(
                ...
                subopt_pess=float(np.mean(subopt_pess_v)),
                subopt_pess_std=float(np.std(subopt_pess_v)),
                ...
                V_low_std={n: float(np.std([r["policies"][n]["V_low"]
                                            for r in recs])) for n in cands})
        all_agg[variant] = agg
